...
| Shared list sync | TanStack Query polling + an offline outbox (no sync engine)  |
...
│   └── offline.ts         # offline snapshot cache + durable outbox for checks/quantities
├── server/
...
- **Shopping list** (`/shopping`) aggregates the ingredients of every active
  ...
  Ticking syncs across both members' devices: writes go to Postgres via server
  functions (queued in a durable offline outbox first, so they survive a lost
  signal in the shop), and every open list polls the server snapshot every few
  seconds while visible, so the other phone catches up within moments.
